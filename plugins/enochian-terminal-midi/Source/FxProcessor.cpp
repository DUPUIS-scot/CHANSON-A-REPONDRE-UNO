...
void EnochianTerminalFxAudioProcessor::processBlock(juce::AudioBuffer<float>& b,juce::MidiBuffer& midi) {
  ...
  const auto drive=parameters.getRawParameterValue("drive")->load()*5.f;
  wetGain.setTargetValue(juce::Decibels::decibelsToGain(parameters.getRawParameterValue("output")->load()));
  for(int c=0;c<b.getNumChannels();++c) for(int i=0;i<b.getNumSamples();++i) { auto dry=b.getSample(c,i); auto wet=std::tanh(dry*(1.f+drive)); b.setSample(c,i,dry+(wet-dry)*parameters.getRawParameterValue("fxMix")->load()*wetGain.getNextValue()); }
}
