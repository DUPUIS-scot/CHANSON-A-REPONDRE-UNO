...
// The exterior master must be centered only in X/Z. Vertically centering the
// GLB puts roughly half of the district below world ground Y=0.
const environmentNeedle = `  root.position.sub(center);\n  root.updateMatrixWorld(true);`;
if (src.includes(environmentNeedle)) {
  src = src.replace(environmentNeedle, `  root.position.x -= center.x;\n  root.position.z -= center.z;\n  root.position.y += -box.min.y + 0.02;\n  root.updateMatrixWorld(true);`);
  changed = true;
}
...
if (!changed) {
  console.log('LUBIAK ground alignment and terrain gravity already installed.');
  process.exit(0);
}

fs.writeFileSync(path, src);
console.log('Applied LUBIAK ground-Y alignment plus GLB-surface gravity.');
