...
const exact = '/assets/assets/models/textured-glb-comparison/LUBIAK_REASSEMBLED_MODULAR_WEB.glb?v=20260903-modular-v1';
...
js = js.replace(/\/assets\/assets\/models\/textured-glb-comparison\/LUBIAK_REASSEMBLED_MODULAR_WEB\.glb\?v=[^'"\s]+/g, exact);
js = js.replace(/const candidates = \[[\s\S]*?\];/, `const candidates = [\n    { url: '${exact}', label: 'LOADING LUBIAK MODULAR MASTER', finish: 'ENTER LUBIAK', timeoutMs: 32000 },\n  ];`);
...
if ((js.match(/LOADING LUBIAK MODULAR MASTER/g) || []).length !== 1) throw new Error('Expected exactly one LUBIAK modular environment candidate');
if (!js.includes(exact)) throw new Error('Exact LUBIAK modular URL was not installed');
fs.writeFileSync(runtimePath, js);

let html = fs.readFileSync(htmlPath, 'utf8');
// Remove the old fetch interception/alias layer entirely. The runtime requests the comparison GLB directly for controlled live testing.
...
html = html.replace(/\/assets\/assets\/models\/textured-glb-comparison\/LUBIAK_master_optimized_4_ELEMENT_PBR_WEB_OPTIMIZED\.glb\?v=[^'"\s]+/g, exact);
...
console.log('LUBIAK now has one direct environment wire only: LUBIAK_REASSEMBLED_MODULAR_WEB.glb');
