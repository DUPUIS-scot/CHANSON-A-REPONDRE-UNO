...
for(let r=0;r<ROWS;r++){
  ...
  for(let c=0;c<COLS;c++){
    ...
    const wave=(Math.sin(c*0.52)*Math.cos(r*0.41))*0.38;
    const ridge=(Math.sin(c*0.27+r*0.19)+Math.sin(c*0.11-r*0.31))*0.16;
    const bowl=-0.18*(nx*nx+nz*nz);
...
const json={asset:{version:'2.0',generator:'CHANSON A REPONDRE UNO megapole signal ground'},scene:0,scenes:[{nodes:[0]}],nodes:[{mesh:0,name:'MEGAPOLE_SIGNAL_GROUND'}],meshes:[{primitives:[{attributes:{POSITION:0},indices:1,material:0}]}],materials:[{name:'AshCoalEmissive',doubleSided:true,pbrMetallicRoughness:{baseColorFactor:[0.018,0.012,0.01,1],metallicFactor:0.05,roughnessFactor:0.96},emissiveFactor:[1.0,0.12,0.015],extensions:{KHR_materials_emissive_strength:{emissiveStrength:2.8}}}],extensionsUsed:['KHR_materials_emissive_strength'],buffers:[{byteLength:bin.length}],bufferViews:[{buffer:0,byteOffset:0,byteLength:posBuf.length,target:34962},{buffer:0,byteOffset:posBuf.length,byteLength:idxBuf.length,target:34963}],accessors:[{bufferView:0,componentType:5126,count:positions.length/3,type:'VEC3',min:[-width/2,-1.0,-depth/2],max:[width/2,1.0,depth/2]},{bufferView:1,componentType:5125,count:indices.length,type:'SCALAR'}]};
