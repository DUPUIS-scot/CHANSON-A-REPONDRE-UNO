(()=>{
  ...
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.authoritativeRuntime==='v3')return true;
      d.documentElement.dataset.authoritativeRuntime='v3';
      ...
      const wrap=()=>{if(!loopEnabled||loopEnd==null)return;const now=performance.now();if(now-lastWrap<80)return;lastWrap=now;try{master.currentTime=loopStart}catch(_){};try{d.getElementById('log')?.prepend(Object.assign(d.createElement('div'),{className:'signal-flow-line',textContent:'LOOP CYCLE · '+fmt(loopStart)+' → '+fmt(loopEnd)}))}catch(_){}};
      ...
      // Sole stem UI authority over the existing native Web Audio graph.
      ...
      const nativeMasterHandler=stemMaster.onclick,nativeRowHandlers=new Map(rows().map(b=>[b,b.onclick])),nativeRangeHandlers=new Map(ranges().map(r=>[r,r.oninput]));
      ...
      const setStemStatus=state=>{lastStemState=state;stemMaster.classList.toggle('stem-loading',state==='loading');stemMaster.classList.toggle('stem-fallback',state==='fallback');const routed=state==='on'||state==='mix';stemMaster.classList.toggle('active',routed);stemMaster.setAttribute('aria-pressed',String(routed));stemMaster.textContent=state==='loading'?'STEMS LOADING':state==='fallback'?'STEMS FALLBACK':state==='off'?'STEMS OFF':state==='mix'?'STEMS MIX':'STEMS ON'};
      const refresh=()=>{if(!stemEnabled){setStemStatus('off');return}if(master.paused){setStemStatus(isCustomMix()?'mix':'on');return}stemReady=allStemMediaRunning();setStemStatus(stemReady?(isCustomMix()?'mix':'on'):'loading')};
      ...
      const waitForNativeStart=async()=>{activating=true;setStemStatus('loading');prepare();for(let i=0;i<30;i++){await new Promise(resolve=>w.setTimeout(resolve,50));if(allStemMediaRunning()){stemReady=true;align(true);refresh();activating=false;return true}}stemReady=false;setStemStatus('fallback');activating=false;return false};

      stemMaster.addEventListener('click',async e=>{
        e.preventDefault();e.stopPropagation();if(activating)return;
        if(stemEnabled){activating=true;try{await callNativeMaster()}catch(_){};stemEnabled=false;stemReady=false;setStemStatus('off');activating=false}
        else{activating=true;setStemStatus('loading');try{await callNativeMaster();stemEnabled=true}catch(_){stemEnabled=false}activating=false;if(stemEnabled&&!master.paused)await waitForNativeStart();else refresh()}
      });
      rows().forEach(b=>b.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const fn=nativeRowHandlers.get(b);if(typeof fn==='function'){const r=fn.call(b,e);if(r&&typeof r.then==='function')await r}refresh()}));
      ranges().forEach(r=>r.addEventListener('input',async e=>{const fn=nativeRangeHandlers.get(r);if(typeof fn==='function'){const x=fn.call(r,e);if(x&&typeof x.then==='function')await x}const out=isolator.querySelector(`[data-stem-value="${r.dataset.stemRange}"]`);if(out)out.textContent=Math.round(parseFloat(r.value)||0)+'%';refresh()}));
      if(play)play.addEventListener('pointerdown',prepare,true);
      // Native play listener is registered before this controller. Observe its startup instead of toggling it again.
      master.addEventListener('play',()=>{if(stemEnabled&&!activating)waitForNativeStart()});
      master.addEventListener('pause',()=>{stemReady=false;refresh()});master.addEventListener('seeking',()=>align(true));master.addEventListener('seeked',()=>align(true));master.addEventListener('ratechange',()=>align(false));
      const drift=()=>{if(stemEnabled&&!master.paused&&!activating){align(false);stemReady=allStemMediaRunning();if(stemReady)refresh()}w.setTimeout(drift,250)};w.setTimeout(drift,250);

      prepare();refresh();loopUi();loopTick();
      w.__enochStemAuthority={version:'v3',get state(){return lastStemState},get enabled(){return stemEnabled},get ready(){return stemReady},isCustomMix};
