(()=>{
  const CSS=`@supports (-webkit-touch-callout:none){@media (orientation:landscape) and (max-height:520px) and (pointer:coarse){
    ...
    html.enoch-ios-landscape .stage{box-sizing:border-box!important;min-height:0!important;height:100%!important;overflow:hidden!important;grid-template-rows:58px calc(100dvh - 98px) 40px minmax(286px,1fr)!important;gap:3px!important}
    ...
    html.enoch-ios-landscape .deckarea>.wheelbox.eq{grid-column:1!important;grid-row:1!important;justify-self:center!important;align-self:center!important;transform:translateY(-4px)!important}
    html.enoch-ios-landscape .deckarea>.wheelbox.fx{grid-column:3!important;grid-row:1!important;justify-self:center!important;align-self:center!important;transform:translateY(-4px)!important}
    html.enoch-ios-landscape .deckarea>.wheelbox{width:66px!important;max-width:66px!important;overflow:visible!important}
    html.enoch-ios-landscape .deckarea>.wheelbox .wheel{width:64px!important;height:64px!important;min-width:64px!important;min-height:64px!important}
    html.enoch-ios-landscape .deckarea>.wheelbox .wheel-hint{max-width:66px!important;overflow:hidden!important;text-overflow:clip!important;white-space:nowrap!important}
    html.enoch-ios-landscape .deckarea>.platter-wrap{position:relative!important;grid-column:2!important;grid-row:1!important;left:auto!important;top:auto!important;right:auto!important;bottom:auto!important;transform:translateY(-6px)!important;width:min(40vh,168px)!important;height:min(40vh,168px)!important;max-width:168px!important;max-height:168px!important;margin:0!important;z-index:6!important;align-self:center!important;justify-self:center!important}
    ...
    /* SCREEN 2 — FX PAD | MIX | INSTANT FX | MOD, fully inside one viewport. */
    html.enoch-ios-landscape .target-mod{min-height:0!important;height:40px!important;display:grid!important;grid-template-columns:minmax(0,1fr) 72px!important;align-items:center!important;padding:2px 4px!important;gap:4px!important;overflow:hidden!important}
    html.enoch-ios-landscape .target-mod textarea{height:27px!important;min-height:27px!important;padding:3px 5px!important;font-size:6.5px!important}
    html.enoch-ios-landscape .pad-panel.instant-layout{box-sizing:border-box!important;display:grid!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;grid-template-columns:minmax(0,1fr) 40px minmax(0,1fr) 62px!important;grid-template-rows:40px minmax(0,1fr)!important;gap:3px!important;padding:3px!important}
    ...
    html.enoch-ios-landscape .pad-mode .btn{min-width:0!important;min-height:17px!important;padding:1px!important;font-size:4.3px!important;overflow:hidden!important}
    html.enoch-ios-landscape .pad-panel.instant-layout>.xy-pad{grid-column:1!important;grid-row:2!important;width:100%!important;min-width:0!important;min-height:154px!important;height:100%!important}
    ...
    html.enoch-ios-landscape .mixer-level-title{font-size:4.7px!important}
    html.enoch-ios-landscape .mixer-level-range{min-height:96px!important;width:22px!important;height:100%!important}
    html.enoch-ios-landscape .mixer-level-value{min-width:29px!important;min-height:18px!important;padding:2px!important;font-size:6.5px!important}
    ...
    html.enoch-ios-landscape .instant-fx-grid{display:grid!important;grid-template-columns:minmax(0,1fr)!important;grid-template-rows:repeat(3,minmax(48px,1fr))!important;width:100%!important;height:100%!important;min-width:0!important;min-height:0!important;gap:3px!important}
    html.enoch-ios-landscape .instant-fx-btn{box-sizing:border-box!important;width:100%!important;max-width:100%!important;min-width:0!important;min-height:48px!important;height:100%!important;padding:3px 2px!important;overflow:hidden!important}
    html.enoch-ios-landscape .instant-fx-btn span{font-size:7.5px!important}
    html.enoch-ios-landscape .instant-fx-btn small{font-size:4.5px!important;margin-top:1px!important}
    html.enoch-ios-landscape .mod-depth-box{grid-column:4!important;grid-row:1/3!important;width:100%!important;min-width:0!important;min-height:0!important;overflow:hidden!important;padding:2px!important;gap:2px!important}
    html.enoch-ios-landscape .mod-depth-box .wheel{width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important}
    html.enoch-ios-landscape .mod-depth-box .wheel-value,html.enoch-ios-landscape .mod-depth-box .wheel-hint{font-size:4.5px!important;line-height:1!important}

    html.enoch-ios-landscape .grid>aside:last-child{min-height:0!important;height:100%!important;overflow:hidden!important;grid-template-rows:auto repeat(5,62px) 38px minmax(205px,1fr)!important;gap:3px!important;align-content:start!important}
  ...
  function install(frame){try{const live=frame.contentDocument;const deck=live&&live.getElementById('deck');const d=deck&&deck.contentDocument;if(!d||!d.head||!d.documentElement.classList.contains('enoch-ios-landscape'))return false;['iphoneLandscapeLayoutV2','iphoneLandscapeLayoutV3','iphoneLandscapeLayoutV4','iphoneLandscapeLayoutV5','iphoneLandscapeLayoutV6','iphoneLandscapeLayoutV7'].forEach(id=>d.getElementById(id)?.remove());const s=d.createElement('style');s.id='iphoneLandscapeLayoutV8';s.textContent=CSS;d.head.appendChild(s);d.documentElement.style.scrollSnapType='y proximity';try{d.defaultView.dispatchEvent(new Event('resize'))}catch(_){}return true}catch(_){return false}}
