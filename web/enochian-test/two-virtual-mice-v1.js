(()=>{
  ...
  function install(host){
    try{
      ...
      const pair={enabled:false,dragging:false,last:null,a:{x:.35,y:.58,target:null,start:0,value:0,locked:false},b:{x:.65,y:.58,target:null,start:0,value:0,locked:false}};
      ...
      d.addEventListener('pointerdown',e=>{if(!pair.enabled||e.target===button)return;
        if(cursorHit(pair.a,e.clientX,e.clientY)){pair.a.locked=true;pair.b.locked=false;pair.last={x:e.clientX,y:e.clientY};render();e.preventDefault();e.stopImmediatePropagation();return}
        if(cursorHit(pair.b,e.clientX,e.clientY)){pair.b.locked=true;pair.a.locked=true;pair.last={x:e.clientX,y:e.clientY};render();e.preventDefault();e.stopImmediatePropagation();return}
        pair.dragging=true;pair.last={x:e.clientX,y:e.clientY};pair.moveX=0;pair.moveY=0;acquire(pair.a);acquire(pair.b);outerEvent('pointerdown',pair.a,71);outerEvent('pointerdown',pair.b,72);e.preventDefault();e.stopImmediatePropagation()},{capture:true});
      ...
      d.addEventListener('keydown',e=>{if(e.key==='Escape'&&pair.enabled){pair.dragging=false;pair.last=null;pair.a.locked=false;pair.b.locked=false;pair.a.target=null;pair.b.target=null;render();e.preventDefault()}},{capture:true});
