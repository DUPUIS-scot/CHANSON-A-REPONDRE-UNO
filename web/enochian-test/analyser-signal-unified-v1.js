...
function install(frame){
 const d=deckDoc(frame),w=d?.defaultView,bus=w?.__enochAnalyserBus;if(!d||!w||!bus)return false;if(d.documentElement.dataset.analyserSignalUnified==='v6')return true;const chamber=d.querySelector('.stage .wave')||d.querySelector('.wave');if(!chamber||chamber.tagName==='CANVAS')return false;d.documentElement.dataset.analyserSignalUnified='v6';
 const css=d.createElement('style');css.textContent=`.wave{background:linear-gradient(rgba(48,166,207,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(48,166,207,.025) 1px,transparent 1px),radial-gradient(circle at 50% 52%,rgba(14,111,160,.18),transparent 60%),${SIGNAL_PALETTE.bgDeep}!important;background-size:24px 24px,24px 24px,auto,auto!important}.wave .analyser-3d:not(.analyser-3d-unified){display:none!important;opacity:0!important}.wave > #wave{position:absolute!important;left:4%!important;top:31%!important;width:92%!important;height:16%!important;z-index:2!important;opacity:.30!important;pointer-events:none!important}.wave .analyser-3d-unified{display:block!important;opacity:.98!important;position:absolute!important;inset:0!important;width:100%!important;height:100%!important;z-index:4!important;pointer-events:none!important;filter:drop-shadow(0 0 3px ${SIGNAL_PALETTE.glowSecondary})!important}`;d.head.appendChild(css);chamber.querySelectorAll('.analyser-3d').forEach(c=>{if(!c.classList.contains('analyser-3d-unified')){c.style.setProperty('display','none','important');c.style.setProperty('opacity','0','important')}});chamber.querySelector('.analyser-3d-unified')?.remove();const canvas=d.createElement('canvas');canvas.className='analyser-3d analyser-3d-unified';canvas.setAttribute('aria-hidden','true');chamber.appendChild(canvas);
 ...
 const paint=t=>{
  ...
  lastProjected=pts;canvas.__enochProjected=pts;canvas.dataset.projectedRows=String(pts.length);canvas.dataset.projectedBins=String(pts[0]?.length||0);canvas.dataset.projectionSpace='terrain-vertices-v1';w.__enochAnalyserUnifiedState={version:'v6',source:latestSource,live,frameSerial:lastFrameSerial,grid:{rows:ROWS,bins:BINS},stems:{vocals:stem('vocals'),drums:stem('drums'),bass:stem('bass'),other:stem('other')}};d.documentElement.dataset.analyserUnifiedHealth=live?'live':'idle';d.documentElement.dataset.analyserUnifiedSource=latestSource;
 };
 const pick=(clientX,clientY)=>{const r=canvas.getBoundingClientRect();if(!r.width||!r.height||!lastProjected.length)return null;const tx=(clientX-r.left)*(canvas.width/r.width),ty=(clientY-r.top)*(canvas.height/r.height);let best=null,dist=Infinity;lastProjected.forEach((line,pr)=>line.forEach((q,pb)=>{const dd=(q.x-tx)*(q.x-tx)+(q.y-ty)*(q.y-ty);if(dd<dist){dist=dd;best={bin:pb/(BINS-1)*15,row:(ROWS-1-pr)/(ROWS-1)*15}}}));return best};const api={version:'v6',grid:{rows:ROWS,bins:BINS},invalidate:()=>{},pick,getProjected:()=>lastProjected,get source(){return latestSource},get live(){return performance.now()-latestStamp<500}};w.__enochAnalyser3DUnified=api;w.__enochAnalyser3D=api;
