...
function install(frame){
 ...
 const paint=t=>{
  ...
  lastProjected=pts;w.__enochAnalyserUnifiedState={version:'v4',source:latestSource,live,frameSerial:lastFrameSerial,grid:{rows:ROWS,bins:BINS},stems:{vocals:stem('vocals'),drums:stem('drums'),bass:stem('bass'),other:stem('other')}};d.documentElement.dataset.analyserUnifiedHealth=live?'live':'idle';d.documentElement.dataset.analyserUnifiedSource=latestSource;
 };
 const pick=(clientX,clientY)=>{const r=canvas.getBoundingClientRect();if(!r.width||!r.height||!lastProjected.length)return null;const tx=(clientX-r.left)*(canvas.width/r.width),ty=(clientY-r.top)*(canvas.height/r.height);let best=null,dist=Infinity;lastProjected.forEach((line,pr)=>line.forEach((q,pb)=>{const dd=(q.x-tx)*(q.x-tx)+(q.y-ty)*(q.y-ty);if(dd<dist){dist=dd;best={bin:pb/(BINS-1)*15,row:(ROWS-1-pr)/(ROWS-1)*15}}}));return best};const api={version:'v4',grid:{rows:ROWS,bins:BINS},invalidate:()=>{},pick,getProjected:()=>lastProjected,get source(){return latestSource},get live(){return performance.now()-latestStamp<500}};w.__enochAnalyser3DUnified=api;w.__enochAnalyser3D=api;
