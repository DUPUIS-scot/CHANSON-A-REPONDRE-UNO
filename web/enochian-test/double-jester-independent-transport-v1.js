...
function install(host){
 try{
  ...
  if(!transport){transport=document.createElement('button');transport.type='button';transport.dataset.jesterIndependentTransport='';transport.className='jester-independent-transport';transport.textContent='▶ PLAY 2JESTER';const center=panel.querySelector('.dds-center')||panel;center.insertBefore(transport,center.firstChild)}
  let style=document.getElementById('jester-independent-transport-style');if(!style){style=document.createElement('style');style.id='jester-independent-transport-style';style.textContent='.jester-independent-transport{width:100%;min-height:34px;border:1px solid #63f5cf;border-radius:4px;background:#06110f;color:#a9eee7;font:1000 9px/1 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;letter-spacing:.08em;cursor:pointer;box-shadow:0 0 12px #19c98f44}.jester-independent-transport.active{background:#163c31;color:#e8fff8;box-shadow:0 0 18px #19c98f88}#play.jester-main-hold{border-color:#e6a64b!important;background:#2a1805!important;color:#ffd28a!important;box-shadow:0 0 16px #e6923266!important;text-shadow:0 0 8px #e69232!important}';document.head.appendChild(style)}
  const paint=()=>{transport.classList.toggle('active',state.playing);transport.textContent=state.playing?'❚❚ PAUSE 2JESTER':'▶ PLAY 2JESTER';mainPlay.classList.toggle('jester-main-hold',state.playing||state.mainHeld);if(state.playing||state.mainHeld){mainPlay.title='MAIN ON HOLD · 2JESTER TRANSPORT';mainPlay.setAttribute('aria-label','MAIN ON HOLD · press to return to main transport')}else{mainPlay.removeAttribute('title');mainPlay.removeAttribute('aria-label')}panel.dataset.jesterTransport=state.playing?'playing':'paused'};
  transport.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();void toggle()});
  ...
  const controller={version:VERSION,state,play:playStems,pause:pauseStems,toggle,sync:syncToJesterClock,stopForMain,originalEnforce};
