...
function install(frame){
 ...
 if(d.documentElement.dataset.sharedInputRouter==='v1'&&w.__enochSharedInputRouter?.version==='v1')return true;
 ...
 const state={context:null,source:null,low:null,mid:null,high:null,filter:null,drive:null,delay:null,fb:null,wet:null,dry:null,master:null,analyser:null,filterIndex:0,gainIndex:0,midiAccess:null,midiBound:new Map(),routes:[],lastDeck:'—'};
 ...
 const expose=()=>{
   if(state.context&&state.low){w.__enochMasterInput=state.low;w.__enochLineInDestination=state.low}
   w.__enochAudioGraph={version:'v1',context:state.context,input:state.low,low:state.low,mid:state.mid,high:state.high,filter:state.filter,drive:state.drive,delay:state.delay,feedback:state.fb,wet:state.wet,dry:state.dry,master:state.master,analyser:state.analyser};
 };
 ...
 // Make every terminal-owned source use one context. The native constructor still owns the first context.
 ...
 // Build the canonical playback graph now so LINE IN never falls back to a separate destination.
 try{const p=w.ensureAudio?.();if(p?.catch)p.catch(()=>{})}catch(_){}
 const routePanel=()=>d.querySelector('.enoch-midi-v2');
 const readRoutes=()=>{
   const view=routePanel(),rows=[...(view?.querySelectorAll('.midi-deck')||[])];
   const next=rows.map(row=>({deck:row.dataset.deck||'?',input:row.querySelector('[data-route-input]')?.value||'*',channel:+(row.querySelector('[data-route-channel]')?.value||1)}));
   state.routes=next.length?next:[{deck:'A',input:'*',channel:1},{deck:'B',input:'*',channel:2}];
   rows.forEach(row=>{const r=state.routes.find(x=>x.deck===(row.dataset.deck||'?')),out=row.querySelector('[data-route-state]');if(out&&r)out.textContent=`ROUTED · ${r.input==='*'?'ANY INPUT':r.input} · CH ${r.channel} → SIGNAL / EQ / FX / MASTER`});
   return state.routes;
 };
 ...
 const bindMidi=async()=>{
   if(!navigator.requestMIDIAccess)return false;
   try{if(!state.midiAccess)state.midiAccess=await navigator.requestMIDIAccess();const inputs=[...state.midiAccess.inputs.values()];
     inputs.forEach(input=>{const prior=state.midiBound.get(input.id);if(prior===input)return;input.onmidimessage=e=>handleMidi(input,e);state.midiBound.set(input.id,input)});
     state.midiAccess.onstatechange=()=>{state.midiBound.clear();bindMidi()};
     const view=routePanel();if(view){const opts=inputs.map(x=>`<option value="${x.id}">${x.name||x.manufacturer||'USB MIDI'}</option>`).join('');view.querySelectorAll('[data-route-input]').forEach(sel=>{const keep=sel.value;sel.innerHTML='<option value="*">ANY USB MIDI</option>'+opts;if([...sel.options].some(o=>o.value===keep))sel.value=keep});readRoutes()}
     return true;
 ...
 const routeLineIn=()=>{if(state.low){w.__enochMasterInput=state.low;w.__enochLineInDestination=state.low;syncLineInUi();return true}try{const p=w.ensureAudio?.();if(p?.catch)p.catch(()=>{})}catch(_){}return !!state.low};
 const observe=()=>{bindMidi();readRoutes();routeLineIn();const view=routePanel();if(view&&!view.dataset.routeAuthority){view.dataset.routeAuthority='v1';view.addEventListener('change',()=>{readRoutes();bindMidi()});const toggle=view.querySelector('[data-route-toggle]');toggle?.setAttribute('title','Route USB MIDI device + channel to Deck A/B and terminal signal chain')}const line=d.querySelector('[data-line-in]');if(line&&!line.dataset.sharedRouteHook){line.dataset.sharedRouteHook='1';line.addEventListener('pointerdown',routeLineIn,{capture:true})}};
 const timer=w.setInterval(observe,350);observe();
 d.documentElement.dataset.sharedInputRouter='v1';
 w.__enochSharedInputRouter={version:'v1',state,readRoutes,bindMidi,routeLineIn,get audioGraph(){return w.__enochAudioGraph},destroy(){w.clearInterval(timer)}};
 return true;
}
