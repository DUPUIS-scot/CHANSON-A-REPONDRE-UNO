(()=>{
  ...
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.compositeAnalyser==='v3')return true;
      d.documentElement.dataset.compositeAnalyser='v3';
      ...
      const tick=()=>{
        ...
        const gesture=w.__enochAnalyserGesture||{x:0,y:0,vx:0,vy:0};
        const wave=d.getElementById('wave'),gw=Math.max(1,wave?.clientWidth||1),gh=Math.max(1,wave?.clientHeight||1);
        const glideX=Math.max(-1,Math.min(1,gesture.x/(gw*.28))),glideY=Math.max(-1,Math.min(1,-gesture.y/(gh*.22)));
        const glideSpeed=Math.max(0,Math.min(1,Math.hypot(gesture.vx||0,gesture.vy||0)/1.2));
        const vals=[
          clamp(raw[0]*.72+42*drums+26*instruments+25*low+10*master+10*Math.max(0,-eqMacro)+10*fxMacro+18*Math.max(0,-glideX)+12*glideSpeed),
          clamp(raw[1]*.72+38*vocals+22*instruments+25*mid+10*master+10*modOn*modDepth+7*padX+16*Math.abs(glideX)+10*Math.max(0,glideY)),
          clamp(raw[2]*.72+26*vocals+30*instruments+25*high+10*master+14*fxMacro+10*mix*padY+10*instant+18*Math.max(0,glideX)+12*Math.max(0,glideY)),
          clamp(raw[3]*.76+46*drums+14*loopOn+10*pitch+10*instant+8*modOn*modDepth+26*glideSpeed+10*Math.abs(glideY))
        ];
        ...
        if(now-lastLog>720&&state!==lastState&&!d.getElementById('audio')?.paused){
          ...
          row.textContent='ENOCHIAN FLOW · '+bits.join(' ')+' · V'+Math.round(vocals*100)+' D'+Math.round(drums*100)+' I'+Math.round(instruments*100)+' M'+Math.round(master*100)+' · EQ '+Math.round(low*100)+'/'+Math.round(mid*100)+'/'+Math.round(high*100)+' · FX '+Math.round(fxMacro*100)+' PAD '+Math.round(padX*100)+'/'+Math.round(padY*100)+' MIX '+Math.round(mix*100)+' MOD '+Math.round(modOn*modDepth*100)+' · GLIDE '+Math.round(glideX*100)+'/'+Math.round(glideY*100)+' V'+Math.round(glideSpeed*100);
