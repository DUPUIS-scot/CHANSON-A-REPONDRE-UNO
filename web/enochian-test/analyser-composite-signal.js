(()=>{
  ...
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.compositeAnalyser==='v8')return true;
      d.documentElement.dataset.compositeAnalyser='v8';
      ...
      const tick=()=>{
        ...
        const instant=[...d.querySelectorAll('.instant-fx-btn')].some(active)?1:0,modOn=active(d.getElementById('modActivate'))?1:0,modDepth=Math.max(0,Math.min(1,get('modDepth')/100)),pitch=Math.min(1,Math.abs(get('pitch'))/12);
        const signalMod=active(d.getElementById('signalModToggle'))||w.__enochSignalModulation===true;
        const gesture=w.__enochAnalyserGesture||{},def=gesture.deform||{};
        const defY=signalMod?Math.max(-1,Math.min(1,def.pullY||0)):0,defZ=signalMod?Math.max(-1,Math.min(1,def.pullZ||0)):0,twist=signalMod?Math.max(-1,Math.min(1,def.twist||0)):0;
        const defSpeed=signalMod?Math.max(0,Math.min(1,Math.hypot(def.vY||0,def.vZ||0)*650)):0;
        const defEnergy=signalMod?Math.max(0,Math.min(1,(Math.abs(defY)*.42+Math.abs(defZ)*.38+Math.abs(twist)*.20)*modDepth)):0;
        const vals=[
          clamp(raw[0]*.72+42*drums+26*instruments+25*low+10*master+10*Math.max(0,-eqMacro)+10*fxMacro+24*Math.max(0,-defZ)*modDepth+12*defEnergy+8*defSpeed),
          clamp(raw[1]*.72+38*vocals+22*instruments+25*mid+10*master+10*modOn*modDepth+7*padX+28*Math.max(0,defY)*modDepth+14*Math.abs(twist)*modDepth+8*defEnergy),
          clamp(raw[2]*.72+26*vocals+30*instruments+25*high+10*master+14*fxMacro+10*mix*padY+10*instant+24*Math.max(0,defZ)*modDepth+18*Math.max(0,-defY)*modDepth+16*Math.abs(twist)*modDepth),
          clamp(raw[3]*.76+46*drums+10*pitch+10*instant+8*modOn*modDepth+28*defSpeed+18*defEnergy)
        ];
        ...
        if(now-lastLog>600&&state!==lastState&&!d.getElementById('audio')?.paused){
          lastLog=now;lastState=state;const row=d.createElement('div');row.className='signal-flow-line';
          row.textContent='ENOCHIAN FLOW · '+bits.join(' ')+' · V'+Math.round(vocals*100)+' D'+Math.round(drums*100)+' I'+Math.round(instruments*100)+' M'+Math.round(master*100)+' · EQ '+Math.round(low*100)+'/'+Math.round(mid*100)+'/'+Math.round(high*100)+' · FX '+Math.round(fxMacro*100)+' PAD '+Math.round(padX*100)+'/'+Math.round(padY*100)+' MIX '+Math.round(mix*100)+' MOD '+Math.round(modOn*modDepth*100)+' · SIGNAL MOD '+(signalMod?'SCULPT '+Math.round(defY*100)+'/'+Math.round(defZ*100)+' T'+Math.round(twist*100)+' S'+Math.round(defEnergy*100):'OFF');
          const log=d.getElementById('log');log.prepend(row);while(log.querySelectorAll('.signal-flow-line').length>18){const rows=log.querySelectorAll('.signal-flow-line');rows[rows.length-1]?.remove()}
        }
