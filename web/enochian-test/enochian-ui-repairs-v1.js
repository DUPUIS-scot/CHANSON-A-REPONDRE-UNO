...
function installHints(doc,win){
  ...
  if(doc.documentElement.dataset.contextHintsV7==='1')return;
  doc.documentElement.dataset.contextHintsV7='1';
  let timer=0;
...
function install(host){
  try{
    ...
    if(!d||!w)return false;
    if(!d.getElementById('enochian-ui-repairs-v7-style')){
      const s=d.createElement('style');s.id='enochian-ui-repairs-v7-style';s.textContent='.enoch-context-hint{position:fixed;z-index:2147483646;max-width:230px;padding:5px 7px;border:1px solid #0e6fa0;background:#020a0ff2;color:#a8b5b5;font:700 7px/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.04em;pointer-events:none;box-shadow:0 3px 12px #000b;display:none}.enoch-context-hint.show{display:block}';d.head.appendChild(s)
    }
    if(!outer.getElementById('enochian-ui-repairs-v7-outer-style')){
      const s=outer.createElement('style');s.id='enochian-ui-repairs-v7-outer-style';s.textContent='.enoch-context-hint{position:fixed;z-index:2147483646;max-width:230px;padding:5px 7px;border:1px solid #0e6fa0;background:#020a0ff2;color:#a8b5b5;font:700 7px/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;pointer-events:none;display:none}.enoch-context-hint.show{display:block}';outer.head.appendChild(s)
    }
    const innerHints=[['#twoMixToggle','2MIX · UNO dual virtual mice · activate two independent virtual pointers'],['#signalModToggle','SIGNAL MOD · sculpt the live 3D mesh'],['[data-stem-toggle],[data-stem-range]','STEMS · enable or mix this stem'],['#low','LOW EQ'],['#mid','MID EQ'],['#high','HIGH EQ'],['#modWheel,#modWheelV,[data-mod]','MOD · modulation control'],['[data-fx],#filter,#drive,#delay,#fb,#wet','FX · effect control'],['#play','PLAY / PAUSE'],['#loopToggle,#loop','LOOP'],['#loopIn','LOOP IN'],['#loopOut','LOOP OUT'],['#loopReset,#resetLoop','RESET LOOP'],['#vol','MASTER VOLUME'],['#pitch','PITCH']];
    ...
    installHints(d,w);installHints(outer,outerWin);
    d.documentElement.dataset.enochUiRepairs=VERSION;
    outer.documentElement.dataset.enochUiRepairs=VERSION;
    return true;
