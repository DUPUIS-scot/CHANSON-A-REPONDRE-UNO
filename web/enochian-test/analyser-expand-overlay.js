(()=>{
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.analyserExpandOverlay==='v5')return true;
      d.documentElement.dataset.analyserExpandOverlay='v5';
      ...
      style.textContent=`
        ...
        .analyser-expand-btn{position:absolute;left:8px;top:5px;z-index:10002;min-height:24px;padding:3px 7px;border:1px solid #315b56;border-radius:4px;background:#020706f2;color:#9dece0;font-size:7px;font-weight:800;letter-spacing:.12em;cursor:pointer;touch-action:manipulation}
        ...
        .wave.analyser-expanded .analyser-resize-hint{display:block;position:absolute;right:9px;bottom:68px;z-index:65;color:#6ba99e;font:600 7px monospace;letter-spacing:.08em;pointer-events:none}
      ...
      btn.setAttribute('aria-label','Open floating analyser signal box');
      wave.appendChild(btn);

      const hint=d.createElement('span');
      hint.className='analyser-resize-hint';
      hint.textContent='DRAG TITLE · RESIZE CORNER · DRAG SIGNAL';
      ...
      const clampPosition=()=>{
        ...
        const maxL=Math.max(4,w.innerWidth-r.width-4), maxT=Math.max(4,w.innerHeight-r.height-4);
        const left=Math.min(Math.max(4,parseFloat(wave.style.left)||r.left),maxL);
        const top=Math.min(Math.max(4,parseFloat(wave.style.top)||r.top),maxT);
      ...
      const setExpanded=on=>{
        if(on){
          ...
          const width=Math.min(Math.max(r.width*1.55,560),Math.max(320,w.innerWidth-24));
          const height=Math.min(Math.max(r.height*2.1,340),Math.max(220,w.innerHeight-24));
          wave.classList.add('analyser-expanded');
          wave.style.width=`${width}px`;wave.style.height=`${height}px`;
          wave.style.left=`${Math.max(4,(w.innerWidth-width)/2)}px`;
          wave.style.top=`${Math.max(4,(w.innerHeight-height)/2)}px`;
          btn.textContent='RESTORE';btn.setAttribute('aria-expanded','true');btn.setAttribute('aria-label','Restore analyser to deck');
        }else{
          wave.classList.remove('analyser-expanded','analyser-moving');
          wave.style.left=saved?.left||'';wave.style.top=saved?.top||'';wave.style.width=saved?.width||'';wave.style.height=saved?.height||'';
          btn.textContent='EXPAND';btn.setAttribute('aria-expanded','false');btn.setAttribute('aria-label','Open floating analyser signal box');
        }
        notifyResize();
      };
      ...
      if(title){
        title.addEventListener('pointerdown',e=>{
          if(!wave.classList.contains('analyser-expanded')||e.button>0)return;
          ...
          const move=ev=>{
            const maxL=Math.max(4,w.innerWidth-wave.offsetWidth-4), maxT=Math.max(4,w.innerHeight-wave.offsetHeight-4);
            wave.style.left=`${Math.min(Math.max(4,ev.clientX-dx),maxL)}px`;
            wave.style.top=`${Math.min(Math.max(4,ev.clientY-dy),maxT)}px`;
          };
          const end=ev=>{wave.classList.remove('analyser-moving');try{title.releasePointerCapture(ev.pointerId)}catch(_){};title.removeEventListener('pointermove',move);title.removeEventListener('pointerup',end);title.removeEventListener('pointercancel',end)};
      ...
      w.addEventListener('resize',()=>{clampPosition();notifyResize()});
