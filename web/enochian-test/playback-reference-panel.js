(()=>{
  ...
  function resolve(frame){
    try{const live=frame&&frame.contentDocument,deck=live&&live.getElementById('deck'),d=deck&&deck.contentDocument,w=d&&d.defaultView;return d&&w?{d,w}:null}catch(_){return null}
  }
  function install(frame){
    ...
    if(!audio||!name||!nativeWave||!prev||!play||!next||!track||!pitch||!loopIn||!loopOut)return false;
    const side=name.closest('aside.side')||name.closest('.side');if(!side)return false;
    d.querySelector('.ref-playback')?.remove();
    d.documentElement.dataset.playbackReferencePanel='v3';

    let style=d.getElementById('ref-playback-v3-style');
    if(style)style.remove();style=d.createElement('style');style.id='ref-playback-v3-style';
    style.textContent=`
    ...
    panel.innerHTML=`<div class="ref-playback-head">PLAYBACK</div><div class="ref-playback-track"><div class="ref-cover" aria-hidden="true">⟁</div><div><div class="ref-track-title"></div><div class="ref-track-sub">ENOCHIAN ORAL TRADITIONS</div><div class="ref-meta"><span class="ref-time">00:00 / 00:00</span><span class="ref-bitrate">320 KBPS</span><span class="ref-key">Am</span></div></div></div><div class="ref-track-picker"><label>5 TRACKS</label></div><div class="ref-wave-shell"><canvas class="ref-main-wave"></canvas><div class="ref-loop-region ref-loop-main"></div><div class="ref-playhead ref-main-playhead"></div></div><div class="ref-overview-shell"><canvas class="ref-overview"></canvas><div class="ref-loop-region ref-loop-overview"></div><div class="ref-playhead ref-overview-playhead"></div></div><div class="ref-transport"><button class="ref-btn ref-cue" type="button">CUE</button></div><div class="ref-loop-title">LOOP</div><div class="ref-loop-controls"><div class="ref-bars"><button class="ref-bars-down" type="button">‹</button><div class="ref-bars-value">8 BARS</div><button class="ref-bars-up" type="button">›</button></div></div><div class="ref-pitch-title"><span>PITCH</span><span class="ref-pitch-value">0.00%</span></div><div class="ref-pitch-row"><span>-8</span><input class="ref-pitch" type="range" min="-8" max="8" step="0.05" value="0" aria-label="Pitch percentage"><span>+8</span><button class="ref-reset" type="button">RESET</button></div>`;
    const oldNow=name.closest('.box');if(oldNow)side.insertBefore(panel,oldNow);else side.prepend(panel);
    panel.querySelector('.ref-track-picker').appendChild(track);track.classList.remove('ref-native-hidden');track.hidden=false;
    ...
    if(oldNow)oldNow.classList.add('ref-native-hidden');nativeWave.classList.add('ref-native-hidden');
    ...
    const barsSet=[1,2,4,8,16,32];let barsCount=8,cuePoint=0,peaks=null,decodeToken=0,lastSource='',raf=0;
    const selectionMeta=()=>track.selectedOptions&&track.selectedOptions[0]?track.selectedOptions[0]:null;
    const bpm=()=>Math.max(40,Math.min(240,Number(selectionMeta()?.dataset?.bpm||audio.dataset.bpm||120)||120));
    const syncMeta=()=>{const opt=selectionMeta();title.textContent=name.textContent||opt?.textContent||'CHANSON À RÉPONDRE';sub.textContent=opt?.dataset?.subtitle||audio.dataset.subtitle||'ENOCHIAN ORAL TRADITIONS';bitrate.textContent=opt?.dataset?.bitrate||audio.dataset.bitrate||'320 KBPS';key.textContent=opt?.dataset?.key||audio.dataset.key||'Am';panel.querySelector('.ref-track-picker label').textContent=Math.max(1,track.options.length)+' TRACKS'};

    const sourceUrl=()=>audio.currentSrc||audio.src||'';
    async function decodeTrack(force=false){
      const src=sourceUrl();if(!src||(!force&&src===lastSource&&peaks))return;lastSource=src;const token=++decodeToken;peaks=null;drawAll();
      try{const response=await w.fetch(src,{cache:'force-cache'});if(!response.ok)throw new Error('wave fetch '+response.status);const bytes=await response.arrayBuffer(),AC=w.AudioContext||w.webkitAudioContext;if(!AC)throw new Error('AudioContext unavailable');const ac=new AC(),buffer=await ac.decodeAudioData(bytes.slice(0));try{await ac.close()}catch(_){}if(token!==decodeToken)return;const channels=Array.from({length:buffer.numberOfChannels},(_,i)=>buffer.getChannelData(i)),count=2048,out=new Float32Array(count);for(let i=0;i<count;i++){const a=Math.floor(i*buffer.length/count),b=Math.max(a+1,Math.floor((i+1)*buffer.length/count));let peak=0,step=Math.max(1,Math.floor((b-a)/18));for(const ch of channels)for(let j=a;j<b;j+=step)peak=Math.max(peak,Math.abs(ch[j]||0));out[i]=peak}peaks=out;drawAll()}catch(_){peaks=null;drawAll()}
    }
    ...
    const drawWave=(canvas,progress)=>{const {W,H}=sizeCanvas(canvas),c=canvas.getContext('2d');c.clearRect(0,0,W,H);c.fillStyle='#110504';c.fillRect(0,0,W,H);const draw=color=>{c.strokeStyle=color;c.lineWidth=Math.max(1,W/900);c.beginPath();for(let x=0;x<W;x++){const p=peaks?peaks[Math.min(peaks.length-1,Math.floor(x/W*peaks.length))]:.12+.06*Math.sin(x*.07),amp=Math.max(1,p*H*.46),mid=H*.5;c.moveTo(x,mid-amp);c.lineTo(x,mid+amp)}c.stroke()};draw('#7e2118');c.save();c.beginPath();c.rect(0,0,W*progress,H);c.clip();draw('#ff4d22');c.restore();c.strokeStyle='#4c1611';c.lineWidth=1;c.beginPath();c.moveTo(0,H*.5);c.lineTo(W,H*.5);c.stroke()};
    ...
    const applyPitch=()=>{const pct=Number(refPitch.value)||0,rate=1+pct/100;refPitchValue.textContent=(pct>0?'+':'')+pct.toFixed(2)+'%';pitch.value=String(12*Math.log2(rate));pitch.dispatchEvent(new w.Event('input',{bubbles:true}));if(Math.abs((Number(audio.playbackRate)||1)-rate)>.002)audio.playbackRate=rate};
    ...
    const onTrackChanged=()=>{peaks=null;lastSource='';cuePoint=0;cue.textContent='CUE';w.setTimeout(()=>{syncMeta();syncTime();decodeTrack(true)},0)};
    track.addEventListener('change',onTrackChanged);audio.addEventListener('loadedmetadata',()=>{syncMeta();syncTime();decodeTrack(true)});audio.addEventListener('durationchange',syncTime);audio.addEventListener('timeupdate',syncTime);audio.addEventListener('emptied',()=>{peaks=null;drawAll()});
    const observer=new w.MutationObserver(syncMeta);observer.observe(name,{childList:true,characterData:true,subtree:true});
    const paint=()=>{syncTime();raf=w.requestAnimationFrame(paint)};raf=w.requestAnimationFrame(paint);
    w.addEventListener('resize',drawAll);w.addEventListener('pagehide',()=>{if(raf)w.cancelAnimationFrame(raf);observer.disconnect()},{once:true});
    w.__enochPlaybackReference={version:'v3',decodeTrack,applyBars,get bars(){return barsCount},get trackCount(){return track.options.length}};
    syncMeta();syncTime();syncPitch();decodeTrack(true);return true;
  }
