(()=>{
'use strict';
const VERSION='20260827-multipoint-sculpt-v2',ORANGE='#ff9d34';
...
function install(frame){
 try{
  ...
  const originalPick=w.__enochAnalyser3D.pick?.bind(w.__enochAnalyser3D),projected=()=>w.__enochAnalyser3D.getProjected?.()||[];
  const nearestAnchor=(x,y)=>{const p=projected(),canvas=wave.querySelector('.analyser-3d'),rect=canvas?.getBoundingClientRect();if(!rect||!canvas)return null;let best=null,dist=Infinity;def.anchors.forEach((a,i)=>{const q=p[Math.round(a.row)]?.[Math.round(a.bin)];if(!q)return;const dx=q.x/(canvas.width/rect.width)-(x-rect.left),dy=q.y/(canvas.height/rect.height)-(y-rect.top),dd=Math.hypot(dx,dy);if(dd<dist){dist=dd;best=i}});return dist<22?best:null};
