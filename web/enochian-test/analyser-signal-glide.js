(()=>{
  ...
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.analyserSignalGlide==='v3')return true;
      d.documentElement.dataset.analyserSignalGlide='v3';
      const style=d.createElement('style');style.textContent=`.wave{cursor:grab!important;touch-action:none!important;user-select:none!important}.wave.analyser-signal-dragging{cursor:grabbing!important}.wave canvas,.analyser-3d{will-change:contents!important;transform:none!important;pointer-events:none!important}.analyser-glide-readout{position:absolute;right:8px;top:5px;z-index:64;font-size:7px;letter-spacing:.12em;color:#d5aa63;background:#020706cc;border:1px solid #315b56;border-radius:4px;padding:3px 5px;pointer-events:none;white-space:nowrap}.signal-mod-toggle{position:absolute;left:8px;top:5px;z-index:65;min-height:20px!important;padding:3px 6px!important;font-size:7px!important;letter-spacing:.1em!important}.signal-mod-toggle.active{background:#083128!important;color:#63f5cf!important;border-color:#68d8bd!important;box-shadow:0 0 10px #40e6b433!important}`;d.head.appendChild(style);
      ...
      let mod=d.getElementById('signalModToggle');if(!mod){mod=d.createElement('button');mod.id='signalModToggle';mod.type='button';mod.className='btn signal-mod-toggle';mod.setAttribute('aria-pressed','false');mod.textContent='SIGNAL MOD OFF';wave.appendChild(mod)}
      w.__enochSignalModulation=false;
      if(!mod.dataset.boundV3){mod.dataset.boundV3='1';mod.addEventListener('click',e=>{e.stopPropagation();const on=mod.getAttribute('aria-pressed')!=='true';w.__enochSignalModulation=on;mod.classList.toggle('active',on);mod.setAttribute('aria-pressed',String(on));mod.textContent=on?'SIGNAL MOD ON':'SIGNAL MOD OFF';render()})}
      ...
      const render=()=>{const speed=Math.hypot(state.vx,state.vy),modOn=w.__enochSignalModulation===true;readout.textContent=(state.dragging?'3D SIGNAL · DRAG':speed>.06?'3D SIGNAL · GLIDE':'3D SIGNAL · LIVE')+(modOn?' · MOD':'')};
