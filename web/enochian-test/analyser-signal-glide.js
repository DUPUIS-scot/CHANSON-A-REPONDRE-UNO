(()=>{
  ...
  function install(frame){
    try{
      ...
      if(d.documentElement.dataset.analyserSignalGlide==='v4')return true;
      d.documentElement.dataset.analyserSignalGlide='v4';
      ...
      const state=w.__enochAnalyserGesture={
        version:'v4',dragging:false,mode:'view',pointerCount:0,x:0,y:0,vx:0,vy:0,
      ...
      const beginMode=(e)=>{
        ...
        if(modOn){
          let picked=null;
          try{picked=w.__enochAnalyser3D?.pick?.(e.clientX,e.clientY)||null}catch(_){}
          state.mode='deform';
          if(picked){state.deform.grabBin=picked.bin;state.deform.grabRow=picked.row}
          else {state.deform.grabBin=null;state.deform.grabRow=0}
          state.deform.vY=state.deform.vZ=0;
          wave.classList.add('analyser-signal-sculpting');
        }else{
          state.mode='view';state.view.yawV=state.view.pitchV=0;
          wave.classList.remove('analyser-signal-sculpting');
        }
      };

      if(!mod.dataset.boundV4){
        mod.dataset.boundV4='1';
        mod.addEventListener('pointerdown',e=>e.stopPropagation());
        mod.addEventListener('click',e=>{
          e.stopPropagation();
          const on=mod.getAttribute('aria-pressed')!=='true';
          w.__enochSignalModulation=on;
          mod.classList.toggle('active',on);mod.setAttribute('aria-pressed',String(on));mod.textContent=on?'SIGNAL MOD ON':'SIGNAL MOD OFF';
          state.mode=on?'deform':'view';render();
        });
      }
      ...
      wave.addEventListener('pointerdown',e=>{
        ...
        activeId=e.pointerId;state.dragging=true;lx=e.clientX;ly=e.clientY;lt=performance.now();beginMode(e);
        wave.classList.add('analyser-signal-dragging');
        try{wave.setPointerCapture(activeId)}catch(_){}
        render();
      });

      wave.addEventListener('pointermove',e=>{
        if(!pointers.has(e.pointerId))return;
        pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});state.pointerCount=pointers.size;
        if(pointers.size>=2){
          const dist=Math.max(1,pointerDistance());state.mode='pinch';state.dragging=true;
          state.view.zoom=clamp(pinchZoom*(dist/pinchDistance),.55,2.6);render();return;
        }
        ...
        if(state.mode==='deform'){
          const sy=Math.max(120,wave.clientHeight),sx=Math.max(160,wave.clientWidth);
          const dY=-dy/sy*1.8,dZ=dx/sx*1.8;
          state.deform.pullY=clamp(state.deform.pullY+dY,-1.5,1.5);
          state.deform.pullZ=clamp(state.deform.pullZ+dZ,-1.5,1.5);
          state.deform.twist=clamp(state.deform.twist+dZ*.55,-1.6,1.6);
          state.deform.vY=state.deform.vY*.45+(dY/dt)*.55;
          state.deform.vZ=state.deform.vZ*.45+(dZ/dt)*.55;
        }else{
          const dYaw=dx*.006,dPitch=dy*.005;
          state.view.yaw=clamp(state.view.yaw+dYaw,-1.15,1.15);
          state.view.pitch=clamp(state.view.pitch+dPitch,-.72,.72);
          state.view.yawV=state.view.yawV*.45+(dYaw/dt)*.55;
          state.view.pitchV=state.view.pitchV*.45+(dPitch/dt)*.55;
        }
      ...
      const release=e=>{
        ...
        if(pointers.size===1&&state.mode==='pinch'){
          const [id,p]=pointers.entries().next().value;activeId=id;lx=p.x;ly=p.y;lt=performance.now();state.mode=w.__enochSignalModulation===true?'deform':'view';return;
        }
        if(pointers.size)return;
        if(!state.dragging)return;
        state.dragging=false;wave.classList.remove('analyser-signal-dragging','analyser-signal-sculpting');
        try{if(activeId!==null&&wave.hasPointerCapture(activeId))wave.releasePointerCapture(activeId)}catch(_){}
        activeId=null;render();
        const moving=state.mode==='deform'?(Math.abs(state.deform.vY)>.00002||Math.abs(state.deform.vZ)>.00002):(Math.abs(state.view.yawV)>.00002||Math.abs(state.view.pitchV)>.00002);
        if(moving)raf=w.requestAnimationFrame(momentum);
      };
      ...
      wave.addEventListener('wheel',e=>{
        ...
        e.preventDefault();
        state.view.zoom=clamp(state.view.zoom*Math.exp(-e.deltaY*.0015),.55,2.6);render();
      },{passive:false});

      wave.addEventListener('dblclick',e=>{
        if(e.target.closest('button'))return;
        stop();state.view.yaw=state.view.pitch=0;state.view.zoom=1;state.view.yawV=state.view.pitchV=0;
        if(w.__enochSignalModulation===true){Object.assign(state.deform,{grabBin:null,grabRow:null,pullY:0,pullZ:0,twist:0,vY:0,vZ:0})}
        render();
      });
