  ...
  window.installEnochianConsoleBottom=function(host){
    function apply(){
      try{
        ...
        const enable=async()=>{try{await ensureAudio();state.enabled=true;state.savedMasterMuted=!!masterAudio.muted;masterAudio.muted=false;panel.querySelector('[data-dds-enable]').classList.add('active');panel.querySelector('[data-dds-enable]').textContent='ENGINE ON';launcher.classList.add('active');setStateText('ENGINE ON');if(!masterAudio.paused)await playAll()}catch(_){state.enabled=false;masterAudio.muted=false;setStateText('ENGINE ERROR')}};
        const disable=()=>{state.enabled=false;pauseAll();masterAudio.muted=false;panel.querySelector('[data-dds-enable]').classList.remove('active');panel.querySelector('[data-dds-enable]').textContent='ENGINE OFF';launcher.classList.remove('active');setStateText('READY')};
