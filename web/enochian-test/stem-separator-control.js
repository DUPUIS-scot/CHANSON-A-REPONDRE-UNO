...
function install(frame){
 try{
  ...
  if(stem.dataset.syntheticStemMaster==='1'&&!stem.dataset.masterBound){stem.dataset.masterBound='1';stem.addEventListener('click',()=>{const on=!isOn();setVisual(on);const rows=[...isolator.querySelectorAll('.stem-toggle')];rows.forEach(b=>{const active=b.classList.contains('active')||b.getAttribute('aria-pressed')==='true';if(active!==on)b.click()})})}
  ...
  if(!kill.dataset.stemKillBoundV3){kill.dataset.stemKillBoundV3='1';kill.addEventListener('click',()=>setTimeout(()=>{if(isOn())stem.click();setTimeout(()=>setVisual(false),0)},0))}
  setVisual(isOn());d.documentElement.dataset.stemSeparatorControl='v3';return true;
