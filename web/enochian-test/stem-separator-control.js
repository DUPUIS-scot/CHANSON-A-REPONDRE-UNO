...
function install(frame){
 try{
  ...
  const state=()=>{const r=rows(),n=r.filter(active).length;return !r.length?'off':n===0?'off':n===r.length?'on':'mix'};
  const setVisual=()=>{const s=state(),on=s==='on',mix=s==='mix';stem.classList.toggle('active',on);stem.classList.toggle('partial',mix);stem.setAttribute('aria-pressed',String(on));stem.dataset.stemState=s;stem.textContent=on?'STEMS ON':mix?'STEMS MIX':'STEMS OFF';stem.title=on?'All separated stems active':mix?'Individual stem mix active':'Separated stems off';isolator.classList.toggle('separator-off',s==='off')};
  const setRows=on=>rows().forEach(b=>{if(active(b)!==on)b.click()});
  if(d.documentElement.dataset.stemsDefaultOnV6!=='1'){
    d.documentElement.dataset.stemsDefaultOnV6='1';
    setRows(true);
  }
  if(!stem.dataset.masterBoundV6){stem.dataset.masterBoundV6='1';stem.addEventListener('click',()=>{const turnOn=state()!=='on';setRows(turnOn);setTimeout(setVisual,0)})}
  rows().forEach(b=>{if(!b.dataset.masterSyncBoundV6){b.dataset.masterSyncBoundV6='1';b.addEventListener('click',()=>setTimeout(setVisual,0))}});
  setTimeout(setVisual,0);d.documentElement.dataset.stemSeparatorControl='v6';return true;
