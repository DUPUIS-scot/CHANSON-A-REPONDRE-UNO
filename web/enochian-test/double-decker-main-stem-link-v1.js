...
function install(host){
  try{
    ...
    const stateFor=key=>{
      ...
      return{
        on:button?button.classList.contains('active'):!!state?.on,
        level:range?clamp(Number(range.value)/100):clamp(state?.level),
      };
    };
    ...
    const propagate=()=>{
      ...
      keys.forEach(key=>{
        ...
        api.setStemOn?.('A',key,state.on);
        api.setStemOn?.('B',key,state.on);
        api.setStemLevel?.('A',key,state.level);
        api.setStemLevel?.('B',key,state.level);
      });
    ...
    const runTransition=fn=>{
      ...
      transition=Promise.resolve().then(fn).finally(()=>{transition=null;paint()});
      return transition;
    };
    ...
    if(!linkButton){
      ...
      linkButton.className='stem-jester-toggle stem-jecker-toggle';
      linkButton.dataset.stemJesterToggle='';
      linkButton.dataset.stemJeckerToggle='';
      center?.appendChild(linkButton);
    }else{
      linkButton.classList.add('stem-jester-toggle');
      linkButton.dataset.stemJesterToggle='';
    }
    panel.querySelectorAll('[data-stem-jecker-shuffle],[data-stem-jester-shuffle]').forEach(node=>node.remove());

    const paint=()=>{
      stemMaster.classList.toggle('stem-jester-mode',mode==='jester');
      stemMaster.classList.toggle('stem-jecker-mode',mode==='jester');
      if(mode==='jester'){
        stemMaster.classList.remove('active','stem-loading','stem-fallback');
        stemMaster.setAttribute('aria-pressed','false');
        stemMaster.dataset.stemMasterMode='jester';
        stemMaster.textContent='STEM → 2JESTER';
        stemMaster.title='STEMS MIX drives 2JESTER; native master stems are held off';
      }
      if(linkButton){
        const live=!!api.state?.enabled;
        linkButton.disabled=!!transition;
        linkButton.classList.toggle('active',linked);
        linkButton.textContent=transition?'STEM JESTER …':linked?(live?'UNLINK STEMS MIX':'STEM JESTER READY'):'LINK STEMS MIX';
        linkButton.title=linked?(live?'Stop routing main STEMS MIX changes into live 2JESTER':'STEM JESTER is armed'):'Route main STEMS MIX controls into 2JESTER';
        linkButton.setAttribute('aria-pressed',String(linked));
      }
    };

    if(stemMaster.dataset.stemJesterBound!==VERSION){
      stemMaster.dataset.stemJesterBound=VERSION;
      stemMaster.addEventListener('click',e=>{
        e.preventDefault();
        e.stopImmediatePropagation();
        if(transition)return;
        if(mode==='jester'&&api.state?.enabled){propagate();paint();return}
        void cycleMode();
      },true);
    }
    if(linkButton&&linkButton.dataset.stemJesterBound!==VERSION){
      linkButton.dataset.stemJesterBound=VERSION;
      linkButton.addEventListener('click',e=>{
        e.preventDefault();
        e.stopPropagation();
        if(transition)return;
        void(linked?enterOff():enterJester());
      });
    }
    ...
    keys.forEach(key=>{
      const button=d.querySelector(`[data-stem-toggle="${key}"]`);
      const range=d.querySelector(`[data-stem-range="${key}"]`);
      if(button&&!button.dataset.stemJesterSync){
        button.dataset.stemJesterSync=VERSION;
        button.addEventListener('click',()=>w.setTimeout(propagate,0));
      }
      if(range&&!range.dataset.stemJesterSync){
        range.dataset.stemJesterSync=VERSION;
        range.addEventListener('input',propagate);
      }
    });
    ...
    const shuffleLive=async deckName=>{
      const mask=liveMask();
      const decks=deckName? [deckName]:['A','B'];
      const jobs=[];
      decks.forEach(name=>{
        ...
        deckEl?.querySelectorAll('.dds-slot').forEach(slotEl=>{
          ...
          const slot=api.state?.slots?.[name]?.[stem];
          if(!stem||!mask[stem]||slot?.on===false)return;
          const select=slotEl.querySelector('[data-source],select');
          if(!select)return;
          const next=randomKey(select.value);
          select.value=next;
          jobs.push(Promise.resolve(api.setSource?.(name,stem,next)));
        });
      });
    ...
    ['A','B'].forEach(name=>{
      const button=panel.querySelector(`[data-dds-deck="${name}"]>.dds-deck-shuffle`);
      if(!button)return;
      button.textContent='SHUFFLE '+name;
      if(button.dataset.stemJesterShuffleBound===VERSION)return;
      button.dataset.stemJesterShuffleBound=VERSION;
      button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();void shuffleLive(name)});
    });

    const reconcile=()=>{
      try{
        if(api.state?.enabled&&engine.status?.().enabled)void setNative(false);
        if(linked&&mode!=='jester')mode='jester';
        if(linked)propagate();
        paint();
      }catch(_){}
    };
    const reconcileTimer=w.setInterval(reconcile,500);

    const controller={
      version:VERSION,
      get linked(){return linked},
      get mode(){return mode},
      setLinked(on){void(on?enterJester():enterOff());return !!on},
      setMode(next){void(next==='on'?enterOn():next==='jester'?enterJester():enterOff());return next},
      sync:propagate,
      syncUi:paint,
      shuffle:shuffleLive,
      liveMask,
    ...
    w.__enochStemJester=controller;
    w.__enochStemJecker=controller;
    w.__enochStemDecker=controller;
    ...
    const loadAddon=(key,src,installer,delay=0)=>{
      ...
      const start=()=>{
        ...
        if(!loader){
          ...
          loader.src=src;
          loader.dataset.jeckerAddon=key;
          loader.onload=()=>run();
    ...
    loadAddon('portal-spinner','/enochian-test/double-jester-portal-spinner-v1.js?v=20260828-v2','installEnochianDoubleJesterPortalSpinnerV2');
    ...
    loadAddon('radial-v10','/enochian-test/double-jecker-radial-layout-v1.js?v=20260827-jester-radial-v10','installEnochianDoubleJeckerRadialV1',80);
    ...
    loadAddon('runtime-repair-v11','/enochian-test/double-jecker-runtime-repair-v1.js?v=20260827-jester-authority-v11','installEnochianDoubleJeckerRuntimeRepairV1',140);
    // CI compatibility markers retained for older deployment verifiers:
    void 'double-jecker-runtime-repair-v1.js?v=20260827-no-recenter-v9';
    void "stemJecker==='v10'";

    paint();
    w.addEventListener('pagehide',()=>w.clearInterval(reconcileTimer),{once:true});
    return true;
  }catch(_){return false}
}
