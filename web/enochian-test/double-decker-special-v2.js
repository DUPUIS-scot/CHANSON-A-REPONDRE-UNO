(()=>{
  ...
  function install(host){
    try{
      ...
      const paint=()=>{const on=!!api.state?.enabled;setTakeover(on);const btn=panel.querySelector('[data-dds-enable]');btn?.classList.toggle('active',on);btn?.classList.toggle('dds-engine-live',on);if(btn)btn.textContent=on?'2JECKER LIVE · MAIN HOLD':'START 2JECKER';launcher.classList.toggle('active',on);setMasterHold(on);setMainPlayIndicator(on);w.__enochStemJecker?.syncUi?.()};
