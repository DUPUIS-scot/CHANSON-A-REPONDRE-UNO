(()=>{
  ...
  function install(host){
    try{
      ...
      if(panel.dataset.ddsControls==='v6')return true;
      panel.dataset.ddsControls='v6';
      ...
      panel.querySelectorAll('[data-dds-shuffle]').forEach(b=>{if(b.dataset.ddsBound==='v6')return;b.dataset.ddsBound='v6';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();shuffle(b.dataset.ddsShuffle)})});
      panel.querySelectorAll('[data-dds-deck-shuffle]').forEach(b=>{if(b.dataset.ddsBound==='v6')return;b.dataset.ddsBound='v6';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();shuffleDeck(b.dataset.ddsDeckShuffle)})});
      panel.querySelectorAll('[data-dds-deck]').forEach(deckEl=>{const deckName=deckEl.dataset.ddsDeck;deckEl.querySelectorAll('.dds-slot').forEach(slotEl=>{const stem=slotEl.dataset.slot;if(!stem)return;let button=slotEl.querySelector('[data-dds-stem-toggle]');if(!button){button=document.createElement('button');button.type='button';button.className='btn stem-toggle active';button.dataset.ddsStemToggle='';slotEl.appendChild(button)}button.className='btn stem-toggle';if(button.dataset.ddsBound!=='v6'){button.dataset.ddsBound='v6';button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const slot=slotState(deckName,stem);setStemOn(deckName,stem,slot?.on===false)})}const range=slotEl.querySelector('[data-level]');if(range&&range.dataset.ddsStemBound!=='v6'){range.dataset.ddsStemBound='v6';range.addEventListener('input',()=>{const slot=slotState(deckName,stem);if(slot?.on===false)requestAnimationFrame(()=>applyStemState(deckName,stem))})}applyStemState(deckName,stem)})});
      ...
      if(engineBtn&&engineBtn.dataset.ddsEngineBound!=='v6'){engineBtn.dataset.ddsEngineBound='v6';engineBtn.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();if(api.state?.enabled){api.disable?.();await restoreNormalStems();setMasterHold(false)}else{await suspendNormalStems();await api.enable?.();if(api.state?.enabled){await enforceActivePlayback(true);setMasterHold(true)}else await restoreNormalStems()}paint()},{capture:true})}
      const syncBtn=panel.querySelector('[data-dds-sync]');if(syncBtn&&syncBtn.dataset.ddsSyncBound!=='v6'){syncBtn.dataset.ddsSyncBound='v6';syncBtn.addEventListener('click',async e=>{e.preventDefault();api.sync?.();await enforceActivePlayback(true);syncBtn.classList.add('active');setTimeout(()=>syncBtn.classList.remove('active'),180)})}
      if(master&&master.dataset.ddsPerfBound!=='v6'){master.dataset.ddsPerfBound='v6';master.addEventListener('play',()=>{w.setTimeout(()=>void enforceActivePlayback(true),0);w.setTimeout(()=>void enforceActivePlayback(false),40)});master.addEventListener('seeking',()=>{if(api.state?.enabled)w.setTimeout(()=>void enforceActivePlayback(true),0)});master.addEventListener('ratechange',()=>{if(api.state?.enabled)w.setTimeout(()=>void enforceActivePlayback(false),0)})}
