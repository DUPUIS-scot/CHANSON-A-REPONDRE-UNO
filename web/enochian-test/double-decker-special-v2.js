(()=>{
  ...
  function install(host){
    try{
      ...
      if(panel.dataset.ddsControls==='v4')return true;
      panel.dataset.ddsControls='v4';
      ...
      panel.querySelectorAll('[data-dds-shuffle]').forEach(b=>{if(b.dataset.ddsBound==='v4')return;b.dataset.ddsBound='v4';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();shuffle(b.dataset.ddsShuffle)})});
      panel.querySelectorAll('[data-dds-deck-shuffle]').forEach(b=>{if(b.dataset.ddsBound==='v4')return;b.dataset.ddsBound='v4';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();shuffleDeck(b.dataset.ddsDeckShuffle)})});
      panel.querySelectorAll('[data-dds-deck]').forEach(deckEl=>{const deckName=deckEl.dataset.ddsDeck;deckEl.querySelectorAll('.dds-slot').forEach(slotEl=>{const stem=slotEl.dataset.slot;if(!stem)return;let button=slotEl.querySelector('[data-dds-stem-toggle]');if(!button){button=document.createElement('button');button.type='button';button.className='btn stem-toggle active';button.dataset.ddsStemToggle='';slotEl.appendChild(button)}button.className='btn stem-toggle';if(button.dataset.ddsBound!=='v4'){button.dataset.ddsBound='v4';button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const slot=slotState(deckName,stem);setStemOn(deckName,stem,slot?.on===false)})}const range=slotEl.querySelector('[data-level]');if(range&&range.dataset.ddsStemBound!=='v4'){range.dataset.ddsStemBound='v4';range.addEventListener('input',()=>{const slot=slotState(deckName,stem);if(slot?.on===false)requestAnimationFrame(()=>applyStemState(deckName,stem))})}applyStemState(deckName,stem)})});
      const setMasterHold=on=>{try{if(on){if(api.state&&typeof api.state.savedMasterMuted!=='boolean')api.state.savedMasterMuted=!!api.audio?.muted;if(api.audio)api.audio.muted=true}else if(api.audio&&api.state)api.audio.muted=!!api.state.savedMasterMuted}catch(_){};if(holdLine){holdLine.classList.toggle('active',!!on);holdLine.textContent=on?'MASTER HOLD · CLOCK RUNNING':'MASTER LIVE'}};
      ...
      if(engineBtn&&engineBtn.dataset.ddsEngineBound!=='v4'){engineBtn.dataset.ddsEngineBound='v4';engineBtn.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();if(api.state?.enabled){api.disable?.();setMasterHold(false)}else{await api.enable?.();setMasterHold(true)}paint()},{capture:true})}
      const syncBtn=panel.querySelector('[data-dds-sync]');if(syncBtn&&syncBtn.dataset.ddsSyncBound!=='v4'){syncBtn.dataset.ddsSyncBound='v4';syncBtn.addEventListener('click',e=>{api.sync?.();syncBtn.classList.add('active');setTimeout(()=>syncBtn.classList.remove('active'),180)})}
      ...
      api.version='v4';api.shuffle=shuffle;api.shuffleDeck=shuffleDeck;api.setStemOn=setStemOn;api.toggleStem=(deckName,stem)=>{const slot=slotState(deckName,stem);return setStemOn(deckName,stem,slot?.on===false)};api.setMasterHold=setMasterHold;
      return true;
