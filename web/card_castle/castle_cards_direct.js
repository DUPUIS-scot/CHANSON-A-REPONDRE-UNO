(() => {
  ...
  function renderCards(THREE,runtime,payload){
    ...
    group.visible=document.body.dataset.sceneMode!=='interior';document.body.dataset.directCardPreviewCount=String(group.children.length);document.body.dataset.directCardPreviewMode='all-decks-rampart-textures-v42';document.body.dataset.directCardTextureLoaded='0';document.body.dataset.directCardTextureFailures='0';pump();
    if(!group.userData.sceneObserver){const syncVisibility=()=>{group.visible=document.body.dataset.sceneMode!=='interior';};group.userData.sceneObserver=new MutationObserver(syncVisibility);group.userData.sceneObserver.observe(document.body,{attributes:true,attributeFilter:['data-scene-mode']});}
  }
