...
if (!window.__castleBrightLabUiPatchInstalled) {
  ...
  function restoreJester(r,mode){
    ...
    if(!j)return;
    const show=mode==='exterior';
    j.visible=show;
    j.traverse(o=>{if(o.userData?.castleGatekeeper===true)o.visible=show;});
    document.body.dataset.castleJesterVisibility=show?'exterior-only-v54':`hidden-${mode}-v54`;
  }
  function sync(){const r=window.__castleSearchRuntime;if(!r?.scene||!r?.renderer){if(attempts++<180)setTimeout(sync,100);return;}const mode=document.body.dataset.sceneMode||'exterior',lab=mode==='laboratory';const rig=ensureLabFillRig();if(rig)rig.visible=lab;
    ...
    if(lab){r.renderer.toneMapping=THREE.ACESFilmicToneMapping;r.renderer.toneMappingExposure=2.05;r.scene.fog=new THREE.FogExp2(0x182630,.0015);if(canvas)canvas.style.filter='contrast(.94) saturate(.92) brightness(1.18)';document.body.dataset.laboratoryLighting='readable-source-orientation-v54';}
    else if(canvas){canvas.style.filter='';}
    lastMode=mode;
  }
