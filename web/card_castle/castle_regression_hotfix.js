...
if (!window.__castleRegressionHotfixV54Installed) {
  window.__castleRegressionHotfixV54Installed = true;
  ...
  function startLaboratoryAnimations(root, clips) {
    ...
    if (!root || !playable.length) { document.body.dataset.laboratoryAnimationPlayback = 'no-clips-v54'; return; }
    ...
    document.body.dataset.laboratoryAnimationPlayback = 'looping-v54';
  ...
  if (!GLTFLoader.prototype.__laboratoryHotfixV54) {
    GLTFLoader.prototype.__laboratoryHotfixV54 = true; const originalLoad=GLTFLoader.prototype.load;
    GLTFLoader.prototype.load=function(url,onLoad,onProgress,onError){
      if(!String(url||'').includes(LAB_TOKEN))return originalLoad.call(this,url,onLoad,onProgress,onError);
      return originalLoad.call(this,url,gltf=>{ const root=gltf?.scene; if(root&&!root.userData.laboratorySourceOrientationV54){root.updateMatrixWorld(true);root.userData.laboratorySourceOrientationV54=true;document.body.dataset.laboratoryOrientation='source-world-orientation-v54';} onLoad?.(gltf); requestAnimationFrame(()=>requestAnimationFrame(()=>{startLaboratoryAnimations(root,gltf?.animations||[]);hideLaboratoryCards();bindLaboratoryVideo();})); },onProgress,onError);
    };
  }

  function hideLaboratoryCards(){const scene=window.__castleSearchRuntime?.scene;if(!scene)return;const active=mode()==='laboratory';scene.children.forEach(child=>{if(!child?.isGroup)return;let has=false;child.traverse?.(o=>{if(o?.userData?.card)has=true});if(has&&active)child.visible=false;});}
  ...
  function bindLaboratoryVideo(){
    ...
    const map=ensureVideo();boundVideoMeshes=targets;targets.forEach(mesh=>{mesh.material=new THREE.MeshBasicMaterial({name:`${mesh.name||'laboratory-video'}-v54`,map,color:0xffffff,side:THREE.DoubleSide,toneMapped:false});mesh.material.needsUpdate=true;mesh.visible=true;});
    document.body.dataset.bureauVideoScreenCount=String(targets.length);document.body.dataset.bureauVideoState='three-surfaces-central-display-v54';syncVideoPlayback();return true;
  }

  function syncVideoPlayback(){if(!video)return;if(mode()==='laboratory')video.play().then(()=>document.body.dataset.bureauVideoPlayback='playing-v54').catch(e=>document.body.dataset.bureauVideoError=String(e?.message||e));else video.pause();}
