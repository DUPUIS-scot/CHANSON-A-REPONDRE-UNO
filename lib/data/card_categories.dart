...
CardCategoryDefinition cardCategoryForStableCard(
  String cardId,
  String? currentCategory,
) {
  if (isKnownCardCategory(currentCategory)) {
    return cardCategoryFor(currentCategory);
  }
  final match = RegExp(r'final-84-(\d+)$').firstMatch(cardId);
  final sequence = int.tryParse(match?.group(1) ?? '');
  return sequence == null
      ? defaultCardCategory
      : cardCategoryAt((sequence - 1).clamp(0, 83));
}
