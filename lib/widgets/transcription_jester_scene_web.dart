...
class TranscriptionJesterScene extends StatefulWidget {
  ...
  static void setOverlayVisible(bool visible) {
    for (final element in html.document.querySelectorAll('[data-transcription-jester-canvas="true"]')) {
      if (!visible) {
        element.style.visibility = 'hidden';
        continue;
      }
      final isPuppet = element.dataset['transcriptionPuppet'] == 'true';
      element.style.visibility = _pendingPuppetEnabled ? (isPuppet ? 'visible' : 'hidden') : (isPuppet ? 'hidden' : 'visible');
    }
  ...
  static void _ensurePuppetScript() {
    ...
    if (existing != null) {
      Timer(Duration.zero, () { _pushPendingPuppetToJs(); _pushPendingPuppetCardToJs(); });
      return;
    }
    final script = html.ScriptElement()
      ..type = 'module'
      ..src = 'transcription_puppet.js?v=20260822g'
      ..dataset['transcriptionPuppetModule'] = 'true';
    script.onLoad.listen((_) { _pushPendingPuppetToJs(); _pushPendingPuppetCardToJs(); });
  ...
  static void _ensurePatchScript() {
    ...
    final existing = html.document.querySelector('script[data-transcription-jester-mode-patch="true"]');
    if (existing != null) { Timer(Duration.zero, _pushPendingJesterModeCardToJs); return; }
    final script = html.ScriptElement()
      ..type = 'module'
  ...
  static void _pushPendingCardToJs() {
    final cardId = _pendingCardId, imagePath = _pendingImagePath;
  ...
  static void _pushPendingPuppetCardToJs() {
    final cardId = _pendingCardId, imagePath = _pendingImagePath;
    if (cardId == null || imagePath == null) return;
    try { _setTranscriptionPuppetCard(cardId, imagePath); } catch (_) {}
  }
...
class _TranscriptionJesterSceneState extends State<TranscriptionJesterScene> {
  ...
  void initState() {
    ...
    for (final delay in const [Duration.zero, Duration(milliseconds: 250), Duration(milliseconds: 750), Duration(milliseconds: 1500), Duration(milliseconds: 3000)]) {
