          ...
          DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [Color(0xFF200906), Color(0xFF100503), Color(0xFF050201)],
              ),
            ),
          ),
          Image(
            image: AssetImage('assets/images/transcription_gothic_background.jpg'),
            fit: BoxFit.cover,
          ...
          DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [Color(0x10000000), Color(0x24080302), Color(0x76080302), Color(0xD9080302)],
                stops: [0, .42, .74, 1],
              ),
            ),
          ),
              ...
              child: DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [Color(0x00000000), Color(0x08000000), Color(0x36080302), Color(0x8A080302)],
                    stops: [0, .50, .78, 1],
                  ),
                ),
              ),
            ...
            child: LayoutBuilder(
              builder: (context, box) {
                ...
                return SingleChildScrollView(
                  padding: EdgeInsets.fromLTRB(compact ? 18 : 36, compact ? 430 : 470, compact ? 18 : 36, 36),
                  child: Center(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (!canUseAi || !ai.isConfigured || ai.error != null) ...[
                            ...
                            _StatusBar(
                              message: ai.error != null ? 'Sorry, not available for the moment. Cheers !' : !ai.isConfigured ? 'AI is not configured for this build.' : 'Guest AI session unavailable. Retry the anonymous session or reload the app.',
                              action: !ai.isConfigured ? TextButton.icon(onPressed: () => context.go(AppRoutes.settings), icon: const Icon(Icons.settings_outlined), label: const Text('Settings')) : !canUseAi ? TextButton.icon(onPressed: () => requireRealAuthentication(context, featureName: 'Card Transcription'), icon: const Icon(Icons.refresh_rounded), label: const Text('Retry')) : null,
                            ),
                          ],
                          if (ai.isLoading) ...[
                            const SizedBox(height: 14),
                          ...
                          if (compact)
                            Column(children: [
                              _TranscribeButton(style: _primaryButtonStyle(), enabled: ai.isConfigured && !ai.isLoading && canUseAi, onPressed: _transcribe),
                              const SizedBox(height: 14),
                              _DiscussButton(style: _secondaryButtonStyle(), enabled: hasText, onPressed: () => context.go(AppRoutes.cardChat(card.id))),
                            ])
                          else
                            Row(children: [
                              Expanded(child: _TranscribeButton(style: _primaryButtonStyle(), enabled: ai.isConfigured && !ai.isLoading && canUseAi, onPressed: _transcribe)),
                              const SizedBox(width: 18),
                              Expanded(child: _DiscussButton(style: _secondaryButtonStyle(), enabled: hasText, onPressed: () => context.go(AppRoutes.cardChat(card.id)))),
                            ]),
                        ],
                      ),
                    ),
...
class _TranscriptionPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(color: const Color(0xD80A0806), borderRadius: BorderRadius.circular(24), border: Border.all(color: _gold, width: 1.25), boxShadow: const [BoxShadow(color: Color(0xA0000000), blurRadius: 28, offset: Offset(0, 14))]),
        child: Column(children: [
          ...
          const SizedBox(height: 18),
          Align(alignment: Alignment.centerLeft, child: Text(text, style: TextStyle(color: hasText ? _cream : const Color(0xAAFFE8B4), fontSize: 21, height: 1.42, fontStyle: hasText ? FontStyle.normal : FontStyle.italic))),
        ]),
      );
}
...
class _TranscribeButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => FilledButton(style: style, onPressed: enabled ? onPressed : null, child: const _ActionButtonLabel(icon: Icons.document_scanner_rounded, title: 'TRANSCRIBE CARD', subtitle: 'Select a card and extract the text'));
}
...
class _DiscussButton extends StatelessWidget {
  ...
  final VoidCallback onPressed;
  @override
  Widget build(BuildContext context) => OutlinedButton(style: style, onPressed: enabled ? onPressed : null, child: const _ActionButtonLabel(icon: Icons.forum_rounded, title: 'DISCUSS WITH AI', subtitle: 'Chat about this transcription'));
}
