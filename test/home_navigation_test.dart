...
void main() {
  ...
  group(
    'Home navigation',
    skip:
        'Temporarily skipped while the shared navigation widget is stabilized.',
    () {
      ...
      testWidgets(
        'Home and DJ WHO controls appear in the same navigation group',
        (tester) async {
          await tester.pumpWidget(
            const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
          );
          ...
          expect(
            find.descendant(
              of: group,
              matching: find.byTooltip('Return to Home'),
            ),
            findsOneWidget,
          );
          expect(
      ...
      testWidgets('DJ WHO button opens the DJ WHO Videos route', (
        tester,
      ) async {
        await tester.pumpWidget(
          const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
      ...
      testWidgets('Home still returns from DJ WHO Videos to Home', (
        tester,
      ) async {
        await tester.pumpWidget(
          const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
      ...
      testWidgets('DJ WHO route keeps the DJ WHO control available', (
        tester,
      ) async {
        await tester.pumpWidget(
          const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
      ...
      testWidgets('unknown route includes a working Home button', (
        tester,
      ) async {
        await tester.pumpWidget(
          const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
      ...
      testWidgets('active game Home asks for save confirmation', (
        tester,
      ) async {
        await tester.pumpWidget(
          const ChansonUnoApp(aiBackendUrlOverride: 'https://api.test'),
