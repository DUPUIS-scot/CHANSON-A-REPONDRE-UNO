...
void main() {
  ...
  group('restored app integration', () {
    ...
    test('restored routes and local Search castle entrypoint remain present', () {
      ...
      expect(castle, contains('../vendor/three.module.js')); expect(castle, contains('assets/assets/models/castle_exterior.glb')); expect(castle, contains("from '../vendor/GLTFLoader.js'")); expect(castle, isNot(contains('unpkg.com'))); expect(castle, contains('LONG_PRESS_MS=600')); expect(castle, contains('longPressTimer:0')); expect(directCards, contains('cardSelected')); expect(directCards, contains('cardLongPressed')); expect(castleHost.replaceAll(RegExp(r'\s+'), ''), contains("'type':'focusCard'")); expect(castle, contains('dragging:false')); expect(castle, contains('document.body.dataset.cardCount')); expect(castle, contains('deriveSurfaceAnchors(84)')); expect(castle, contains('document.body.dataset.surfaceAnchorCount')); expect(bridge, contains("type !== 'setCards'")); expect(bridge, contains('imagePath')); expect(bridge, contains('rectoUrl'));
    });
