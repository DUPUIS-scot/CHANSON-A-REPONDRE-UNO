...
void main() {
  ...
  test('transcription handoff uses canonical card plus direct preview image', () {
    ...
    expect(
      prompt,
      contains(
        'Read CARD IMAGE directly. If that URL is unavailable or returns 404, '
        'use CARD IMAGE FALLBACK instead.',
      ),
    );
    ...
    expect(prompt, contains('CARD IMAGE FALLBACK:'));
    expect(
      prompt,
      contains('No extracted card text is available in the app for this card.'),
    );
    expect(
      prompt,
      contains('Transcribe the first accessible image URL above directly.'),
    );
  ...
  test('DIY handoff uses canonical card plus direct preview image', () {
    ...
    expect(
      prompt,
      contains('Discuss the card from the first accessible image URL above.'),
    );
    ...
    expect(
      prompt,
      isNot(contains('requires a transcription generated by the app')),
    );
  });
  ...
  test('existing transcription is supplied directly to the AI handoff', () {
    ...
    expect(
      prompt,
      contains(
        'Discuss this card using the supplied card text as the primary textual source',
      ),
    );
  });
